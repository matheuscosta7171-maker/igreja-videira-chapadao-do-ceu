...
-- Complementos do painel final. Todos são aditivos e preservam dados existentes.
...
alter table public.cells add constraint cells_status_check check(status in ('active','inactive','archived'));
...
alter table public.cell_reports add constraint cell_reports_status_check check(status in ('draft','submitted','reviewed','returned','archived'));
...
alter table public.church_schedule add constraint church_schedule_status_check check(status in ('scheduled','cancelled','completed','archived'));
...
create policy profiles_update_admin on public.profiles for update to authenticated using(public.can_manage_content()) with check(public.can_manage_content());
drop policy if exists roles_select on public.user_roles;
create policy roles_select on public.user_roles for select to authenticated using(user_id=auth.uid() or public.can_view_all_reports());
...
begin
  if target_role not in ('superadmin','admin','pastor','leader','member') then raise exception 'Perfil inválido'; end if;
  if not public.has_any_role(array['superadmin','admin']) then raise exception 'Sem permissão'; end if;
  if target_role in ('superadmin','admin') and not public.is_superadmin() then raise exception 'Somente superadmin pode atribuir perfis administrativos'; end if;
  if remove_role then
    delete from public.user_roles where user_id=target_user and role=target_role;
  else
    insert into public.user_roles(user_id,role,assigned_by) values(target_user,target_role,auth.uid()) on conflict(user_id,role) do update set assigned_by=excluded.assigned_by;
  end if;
end;
$$;
...
create policy groups_public_read on public.church_groups for select to anon,authenticated using((published and active and not archived) or public.can_manage_content());
drop policy if exists groups_admin_all on public.church_groups;
create policy groups_admin_all on public.church_groups for all to authenticated using(public.can_manage_content()) with check(public.can_manage_content());
...
begin
  foreach table_name in array array['user_roles','cells','leader_cells','public_leaders','site_settings','homepage_sections','church_schedule','church_contacts','church_groups','giving_information','construction_projects','construction_milestones','church_addresses','content_pages'] loop
...
declare table_name text;
begin
  foreach table_name in array array['profiles','cells','public_leaders','cell_reports','prayer_requests','pastoral_visits','testimonies','church_schedule','church_contacts','church_groups','content_pages'] loop
...
grant select on public.public_leaders,public.site_settings,public.homepage_sections,public.church_schedule,public.church_contacts,public.church_groups,public.giving_information,public.construction_projects,public.construction_milestones,public.church_addresses,public.content_pages to anon,authenticated;
grant insert on public.prayer_requests,public.pastoral_visits,public.testimonies to anon,authenticated;
grant select,insert,update,delete on public.profiles,public.user_roles,public.cells,public.leader_cells,public.cell_reports,public.prayer_requests,public.pastoral_visits,public.pastor_availability,public.notifications,public.testimonies,public.public_leaders,public.site_settings,public.homepage_sections,public.church_schedule,public.church_contacts,public.church_groups,public.giving_information,public.construction_projects,public.construction_milestones,public.church_addresses,public.content_pages,public.media_files to authenticated;
