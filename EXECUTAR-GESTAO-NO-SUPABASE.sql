-- Igreja Videira — módulo autônomo de gestão
-- Seguro para nova execução: não remove tabelas nem dados existentes.
...
create table if not exists public.cell_reports (
  ...
  status text not null default 'draft' check(status in ('draft','submitted','reviewed','returned','archived')),
...
create table if not exists public.church_groups (
  ...
  image_path text,
  group_url text,
  button_label text not null default 'Entrar no grupo',
  display_order integer not null default 0,
  active boolean not null default true,
  published boolean not null default false,
  archived boolean not null default false,
...
-- Compatibilidade aditiva com instalações parciais anteriores.
alter table public.cells add column if not exists assistant_id uuid references public.profiles(id);
alter table public.cells add column if not exists district text;
alter table public.cells add column if not exists map_url text;
alter table public.cells add column if not exists phone text;
alter table public.cells add column if not exists observations text;
alter table public.cells add column if not exists status text not null default 'active';
alter table public.public_leaders add column if not exists profile_id uuid references public.profiles(id);
alter table public.public_leaders add column if not exists photo_path text;
alter table public.public_leaders add column if not exists is_primary boolean not null default true;
alter table public.public_leaders add column if not exists is_assistant boolean not null default false;
alter table public.cell_reports add column if not exists internal_notes text;
alter table public.cell_reports add column if not exists reviewed_by uuid references public.profiles(id);
alter table public.cell_reports add column if not exists reviewed_at timestamptz;
alter table public.church_schedule add column if not exists image_path text;
alter table public.church_schedule add column if not exists link_url text;
alter table public.church_schedule add column if not exists featured boolean not null default false;
alter table public.church_schedule add column if not exists status text not null default 'scheduled';
alter table public.giving_information add column if not exists pix_key_type text;
alter table public.church_addresses add column if not exists phone text;
alter table public.church_addresses add column if not exists office_hours text;
...
-- Normaliza restrições de instalações parciais sem remover tabelas ou dados.
...
alter table public.cell_reports drop constraint if exists cell_reports_status_check;
alter table public.cell_reports add constraint cell_reports_status_check
  check(status in ('draft','submitted','reviewed','returned','archived')) not valid;

alter table public.church_schedule drop constraint if exists church_schedule_status_check;
alter table public.church_schedule add constraint church_schedule_status_check
  check(status in ('scheduled','cancelled','completed','archived')) not valid;
...
create index if not exists cell_reports_meeting_date_idx on public.cell_reports(meeting_date desc);
create index if not exists cell_reports_leader_idx on public.cell_reports(leader_id,meeting_date desc);
create index if not exists cell_reports_cell_idx on public.cell_reports(cell_id,meeting_date desc);
create index if not exists prayer_requests_status_idx on public.prayer_requests(status,created_at desc);
create index if not exists pastoral_visits_status_idx on public.pastoral_visits(status,desired_date);
...
returns boolean language sql stable security definer set search_path=public as $$
  select exists(
    select 1 from public.leader_cells
    where leader_id=auth.uid() and cell_id=target_cell
  );
$$;

create or replace function public.manage_user_role(target_user uuid,target_role text,remove_role boolean default false)
returns void language plpgsql security definer set search_path=public as $$
begin
  if target_role not in ('superadmin','admin','pastor','leader','member') then
    raise exception 'Perfil inválido';
  end if;
  if not public.has_any_role(array['superadmin','admin']) then
    raise exception 'Sem permissão';
  end if;
  if target_role in ('superadmin','admin') and not public.is_superadmin() then
    raise exception 'Somente superadmin pode atribuir perfis administrativos';
  end if;
  if remove_role then
    delete from public.user_roles where user_id=target_user and role=target_role;
  else
    insert into public.user_roles(user_id,role,assigned_by)
    values(target_user,target_role,auth.uid())
...
begin
  if tg_op='INSERT' then
    ...
    select distinct ur.user_id,'visit_requested','Nova solicitação de visita',
      'Uma nova solicitação pastoral aguarda análise.','pastoral_visit',new.id
    from public.user_roles ur
    where ur.role in ('superadmin','admin','pastor');
  elsif new.status is distinct from old.status and new.submitter_id is not null then
    insert into public.notifications(recipient_id,type,title,message,entity_type,entity_id)
    values(new.submitter_id,'visit_status','Atualização da visita',
      'Sua solicitação pastoral foi atualizada para: '||new.status||'.','pastoral_visit',new.id);
  end if;
  return new;
end;
$$;
...
begin
  record_id=coalesce(
    ...
    to_jsonb(new)->>'leader_id',to_jsonb(old)->>'leader_id',
    to_jsonb(new)->>'section_key',to_jsonb(old)->>'section_key',
    to_jsonb(new)->>'slug',to_jsonb(old)->>'slug',''
  );
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,summary,old_values,new_values)
  values(
    auth.uid(),lower(tg_op),tg_table_name,record_id,tg_op||' em '||tg_table_name,
...
create trigger pastoral_visit_notifications
after insert or update of status on public.pastoral_visits
...
begin
  foreach table_name in array array[
    'user_roles','cells','leader_cells','public_leaders','site_settings',
    'homepage_sections','church_schedule','church_contacts','church_groups',
    'giving_information','construction_projects','construction_milestones',
    'church_addresses','content_pages'
  ] loop
...
begin
  foreach table_name in array array[
    'profiles','cells','public_leaders','cell_reports','prayer_requests',
    'pastoral_visits','testimonies','church_schedule','church_contacts',
    'church_groups','content_pages'
  ] loop
...
using(id=auth.uid() or public.has_any_role(array['superadmin','admin','pastor']));
...
using(user_id=auth.uid() or public.can_view_all_reports());
drop policy if exists roles_superadmin_all on public.user_roles;
create policy roles_superadmin_all on public.user_roles for all to authenticated
using(public.is_superadmin()) with check(public.is_superadmin() and assigned_by=auth.uid());

drop policy if exists cells_public_read on public.cells;
create policy cells_public_read on public.cells for select to anon,authenticated
using((published and active) or public.can_view_all_reports() or public.owns_cell(id));
drop policy if exists cells_admin_all on public.cells;
create policy cells_admin_all on public.cells for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists leader_cells_select on public.leader_cells;
create policy leader_cells_select on public.leader_cells for select to authenticated
using(leader_id=auth.uid() or public.can_view_all_reports());
drop policy if exists leader_cells_admin_all on public.leader_cells;
create policy leader_cells_admin_all on public.leader_cells for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists public_leaders_read on public.public_leaders;
create policy public_leaders_read on public.public_leaders for select to anon,authenticated
using((published and active) or public.can_manage_content());
drop policy if exists public_leaders_admin_all on public.public_leaders;
create policy public_leaders_admin_all on public.public_leaders for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists reports_select on public.cell_reports;
create policy reports_select on public.cell_reports for select to authenticated
using(public.can_view_all_reports() or (leader_id=auth.uid() and public.owns_cell(cell_id)));
drop policy if exists reports_insert on public.cell_reports;
create policy reports_insert on public.cell_reports for insert to authenticated
with check(
  public.can_view_all_reports()
  or (leader_id=auth.uid() and public.owns_cell(cell_id) and created_by=auth.uid())
);
drop policy if exists reports_update on public.cell_reports;
create policy reports_update on public.cell_reports for update to authenticated
using(
  public.can_view_all_reports()
  or (leader_id=auth.uid() and public.owns_cell(cell_id) and status in ('draft','returned'))
)
with check(
  public.can_view_all_reports()
  or (leader_id=auth.uid() and public.owns_cell(cell_id) and status in ('draft','submitted'))
);

drop policy if exists prayer_public_insert on public.prayer_requests;
create policy prayer_public_insert on public.prayer_requests for insert to anon,authenticated
with check(
  status='received' and internal_notes is null
...
using(public.can_view_pastoral_data() or submitter_id=auth.uid());
drop policy if exists prayer_staff_update on public.prayer_requests;
create policy prayer_staff_update on public.prayer_requests for update to authenticated
using(public.can_view_pastoral_data()) with check(public.can_view_pastoral_data());

drop policy if exists visits_public_insert on public.pastoral_visits;
create policy visits_public_insert on public.pastoral_visits for insert to anon,authenticated
with check(
  status='requested' and pastor_id is null
  and (submitter_id is null or submitter_id=auth.uid())
);
drop policy if exists visits_select on public.pastoral_visits;
create policy visits_select on public.pastoral_visits for select to authenticated
using(public.can_view_pastoral_data() or submitter_id=auth.uid());
drop policy if exists visits_staff_update on public.pastoral_visits;
create policy visits_staff_update on public.pastoral_visits for update to authenticated
using(public.can_view_pastoral_data()) with check(public.can_view_pastoral_data());
drop policy if exists availability_staff on public.pastor_availability;
create policy availability_staff on public.pastor_availability for all to authenticated
using(public.can_view_pastoral_data()) with check(public.can_view_pastoral_data());

drop policy if exists testimonies_public_insert on public.testimonies;
create policy testimonies_public_insert on public.testimonies for insert to anon,authenticated
with check(
  status='pending' and featured=false and published_at is null
  and (submitter_id is null or submitter_id=auth.uid())
);
drop policy if exists testimonies_public_read on public.testimonies;
create policy testimonies_public_read on public.testimonies for select to anon,authenticated
using(
  (status='published' and publication_authorized)
  or public.can_manage_content()
  or submitter_id=auth.uid()
);
drop policy if exists testimonies_admin_update on public.testimonies;
create policy testimonies_admin_update on public.testimonies for update to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());
...
create policy homepage_public_read on public.homepage_sections for select to anon,authenticated
using((published and not archived) or public.can_manage_content());
drop policy if exists homepage_admin_all on public.homepage_sections;
create policy homepage_admin_all on public.homepage_sections for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists schedule_public_read on public.church_schedule;
create policy schedule_public_read on public.church_schedule for select to anon,authenticated
using((published and not archived) or public.can_manage_content());
drop policy if exists schedule_admin_all on public.church_schedule;
create policy schedule_admin_all on public.church_schedule for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists contacts_public_read on public.church_contacts;
create policy contacts_public_read on public.church_contacts for select to anon,authenticated
using((published and not archived) or public.can_manage_content());
drop policy if exists contacts_admin_all on public.church_contacts;
create policy contacts_admin_all on public.church_contacts for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists groups_public_read on public.church_groups;
create policy groups_public_read on public.church_groups for select to anon,authenticated
using((published and active and not archived) or public.can_manage_content());
drop policy if exists groups_admin_all on public.church_groups;
create policy groups_admin_all on public.church_groups for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists giving_public_read on public.giving_information;
create policy giving_public_read on public.giving_information for select to anon,authenticated
using(published or public.can_manage_content());
drop policy if exists giving_admin_all on public.giving_information;
create policy giving_admin_all on public.giving_information for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists construction_public_read on public.construction_projects;
create policy construction_public_read on public.construction_projects for select to anon,authenticated
using(published or public.can_manage_content());
drop policy if exists construction_admin_all on public.construction_projects;
create policy construction_admin_all on public.construction_projects for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());
drop policy if exists milestones_public_read on public.construction_milestones;
create policy milestones_public_read on public.construction_milestones for select to anon,authenticated
using(
  exists(select 1 from public.construction_projects p where p.id=project_id and p.published)
  or public.can_manage_content()
);
drop policy if exists milestones_admin_all on public.construction_milestones;
create policy milestones_admin_all on public.construction_milestones for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists address_public_read on public.church_addresses;
create policy address_public_read on public.church_addresses for select to anon,authenticated
using(published or public.can_manage_content());
drop policy if exists address_admin_all on public.church_addresses;
create policy address_admin_all on public.church_addresses for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());

drop policy if exists pages_public_read on public.content_pages;
create policy pages_public_read on public.content_pages for select to anon,authenticated
using(status='published' or public.can_manage_content());
drop policy if exists pages_admin_all on public.content_pages;
create policy pages_admin_all on public.content_pages for all to authenticated
using(public.can_manage_content()) with check(public.can_manage_content());
drop policy if exists media_admin_all on public.media_files;
create policy media_admin_all on public.media_files for all to authenticated
...
create policy audit_superadmin_read on public.audit_logs for select to authenticated
using(public.is_superadmin());

-- 6. Armazenamento público de mídia administrativa
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types)
values(
  'church-public','church-public',true,15728640,
...
create policy church_public_read on storage.objects for select to anon,authenticated
using(bucket_id='church-public');
drop policy if exists church_public_manage on storage.objects;
create policy church_public_manage on storage.objects for all to authenticated
using(bucket_id='church-public' and public.can_manage_content())
with check(bucket_id='church-public' and public.can_manage_content());

-- 7. Grants mínimos; RLS continua obrigatório.
...
grant select on
  public.public_leaders,public.site_settings,public.homepage_sections,
  public.church_schedule,public.church_contacts,public.church_groups,
  public.giving_information,public.construction_projects,
  public.construction_milestones,public.church_addresses,public.content_pages
to anon,authenticated;

grant insert on public.prayer_requests,public.pastoral_visits,public.testimonies
to anon,authenticated;

grant select,insert,update,delete on
  public.profiles,public.user_roles,public.cells,public.leader_cells,
  public.cell_reports,public.prayer_requests,public.pastoral_visits,
  public.pastor_availability,public.notifications,public.testimonies,
  public.public_leaders,public.site_settings,public.homepage_sections,
  public.church_schedule,public.church_contacts,public.church_groups,
  public.giving_information,public.construction_projects,
  public.construction_milestones,public.church_addresses,
  public.content_pages,public.media_files
to authenticated;
